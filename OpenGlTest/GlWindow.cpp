...
GlWindow::GlWindow()
{
	...
	if (!display) {
		throw std::runtime_error(CONTEXT("failed to open X display"));
	}
	...
	if (!glXQueryVersion( display, &glx_major, &glx_minor) ||
		...
		throw std::runtime_error(CONTEXT("invalid GLX version"));
	}
	...
	if (!fbc) {
		throw std::runtime_error(CONTEXT("failed to retrieve a framebuffer config"));
	}
	...
	for (i=0; i<fbcount; ++i) {
		...
		if (vi) {
			...
			std::cout << "  Matching fbconfig " << i << ", visual ID 0x" << std::hex <<
				vi->visualid << std::dec << ": SAMPLE_BUFFERS = " << samp_buf <<
	...
	win = XCreateWindow(display, RootWindow(display, vi->screen), 0, 0, 640, 480, 0, vi->depth, InputOutput, 
	                    vi->visual, CWBorderPixel|CWColormap|CWEventMask, &swa );
	if ( !win ) {
		throw std::runtime_error(CONTEXT("failed to create window"));
	}
	...
	if (ctxErrorOccurred || !ctx) {
		throw std::runtime_error(CONTEXT("failed to create an OpenGL context"));
	}
